    ...
    func deleteLineItemFromDraftOrder(draftOrderId: Int, lineItemId: Int) {
        ...
        if let draftOrderIndex = cart.firstIndex(where: { $0.id == draftOrderId }) {
            ...
            if (cart[draftOrderIndex].line_items.count==1){
                ...
                let targetProductId = cart[draftOrderIndex].line_items[0].product_id
                if let filteredProduct = productDataArray.first(where: { $0.product.id == targetProductId }) {
                    ...
                    if let filteredVariant = filteredProduct.product.variants?.first(where: {
                        $0.id == cart[0].line_items[0].variant_id
                    }) {
                        let inventoryItemId = filteredVariant.inventory_item_id
                        editVariantQuantity(inventory_item_id: inventoryItemId, new_quantity: cart[0].line_items[0].quantity) {
            ...
            if let lineItemIndex = cart[draftOrderIndex].line_items.firstIndex(where: { $0.id == lineItemId }) {
                ...
                let targetProductId = cart[draftOrderIndex].line_items[lineItemIndex].product_id
                if let filteredProduct = productDataArray.first(where: { $0.product.id == targetProductId }) {
                    
                    if let filteredVariant = filteredProduct.product.variants?.first(where: {
                        $0.id == cart[draftOrderIndex].line_items[lineItemIndex].variant_id
                    }) {
                        let inventoryItemId = filteredVariant.inventory_item_id
                        editVariantQuantity(inventory_item_id: inventoryItemId, new_quantity: cart[draftOrderIndex].line_items[lineItemIndex].quantity) {
